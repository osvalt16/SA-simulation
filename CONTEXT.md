...
- `scripts/fetch-c4-markets.js` lit les 56 marches locaux de C4 (comptes `localMarket`, carnets `bids`/`asks`) et ecrit `c4_markets.json`. Le parseur verifie que la lecture consomme EXACTEMENT la taille du compte, sinon le marche est ignore : c'est ce controle qui a permis de trouver le bon alignement (carnets a l'offset 100).
- ETAT DU MARCHE C4 : quasi vide (15 a 20 marches sur 56 ont au moins un ordre, souvent un seul, et surtout des composants plutot que des gisements bruts). Ces prix ne sont PAS exploitables pour optimiser aujourd'hui ; le suivi sert a constituer l'historique.
